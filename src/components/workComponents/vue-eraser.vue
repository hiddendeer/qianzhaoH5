...
export default {
  ...
  methods: {
      ...
      setTimeout(() => {
        ...
        imgObj.onload = function() {
          ...
          setTimeout(() => {
            ...
            _this.numParts =
              Math.floor(_this.height / _this.size) * _this.colParts;
    ...
    evaluatePoint(tx, ty) {
       ...
       */
      var p =
        Math.floor(tx / this.size) + Math.floor(ty / this.size) * this.colParts;
        ...
        if (!this.complete) {
          if (this.ratio / this.numParts >= this.completeRatio) {
            ...
            if (
              ...
              typeof this.completeFunction === "function"
            )
              this.completeFunction(this.ratio / this.numParts);
          } else {
            if (
              this.progressFunction != null &&
              typeof this.progressFunction === "function"
            )
              this.progressFunction(this.ratio / this.numParts);
          }
        }
      }
    },
