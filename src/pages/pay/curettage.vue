    ...
    <view class="fui-scroll__wrap">
      ...
      <scroll-view scroll-y class="fui-scroll__view">
        <div class="p-[40rpx] text-26rpx">
          ...
          <div class="flex">
            <div class="font-bold flex items-center">选择</div>
            <div class="pl-[20rpx]">
              <fui-data-tag
                :modelValue="value2"
                :height="50"
          ...
          <div class="relative">
            <div class="mt-[50rpx]">
              ...
              @click="changeRadio({value: item.uuid})"
                class="w-[90%] min-h-[200rpx] mt-[20rpx] base-filter rounded-[8rpx] flex"
              >
                <div class="flex-[1] flex items-center">
                  <div
                    class="text-[#f76260] font-bold flex justify-around w-[80%]"
                  >
                    <fui-radio
                      color="#FFB703"
                      :scaleRatio="0.8"
                      :value="item.uuid"
                      :checked="item.uuid == selectRadio"
                      @change="changeRadio"
                    ></fui-radio
                    ><span class="text-30rpx font-bold">{{item.discount}} 折</span>
                  </div>
                </div>
...
const getPhoneCoupon = async () => {
  ...
  if (res.errorCode=='') {
    ...
    res.data.forEach(item => {
      if (!item.used) {
        item.calcMoney = value2.value[0] * item.discount/10;
        list.push(item)
      }
    })
